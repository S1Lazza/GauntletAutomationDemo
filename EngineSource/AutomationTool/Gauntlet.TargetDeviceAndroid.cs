...
// To replace into Engine\Source\Programs\AutomationTool\Gauntlet\Platform\Android
	...
	class AndroidAppInstance : IAppInstance
	{
		...
		private bool IsActivityRunning()
		{
			...
			if (bHasExited)
			{
				...
				Log.VeryVerbose("{0}: process exited, Activity running={1}, Activity in foreground={2} ", ToString(), bActivityPresent.ToString(), bActivityExited.ToString());
			}
